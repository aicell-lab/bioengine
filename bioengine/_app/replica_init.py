"""Replica-side artifact materialisation, run before user code is imported.

Installed as a Ray Serve replica's ``runtime_env.worker_process_setup_hook``
so it runs *before* Ray Serve cloudpickle-loads the user's deployment class
(which imports the user's module — requires the source on ``sys.path``).

Removes the worker↔Ray-cluster shared-filesystem assumption baked into the
v0.11.3 builder: the worker no longer ships the user's package via
``runtime_env.py_modules``; each replica fetches it from Hypha's
``create-zip-file`` endpoint directly. With many replicas on the same node,
an ``fcntl`` lock serialises the download so it happens once per node.

Reads its configuration from env_vars set by the BioEngine worker:

* ``BIOENGINE_APP_DIR``                — ``<apps_workdir>/<worker-ws>-<app-id>``
* ``BIOENGINE_ARTIFACT_DOWNLOAD_URL``  — full ``create-zip-file`` URL incl. version
* ``BIOENGINE_ARTIFACT_VERSION``       — used for the cache-invalidation marker
* ``BIOENGINE_ARTIFACT_DOWNLOAD_TOKEN``— short-TTL read-only Hypha token, or empty
"""
...
def _ensure_source(app_dir: Path, version: str, logger: logging.Logger) -> Path:
    ...
    Single ``fcntl`` lock per node serialises concurrent same-node replica
    starts; the second replica sees the up-to-date ``.version`` marker and
    skips the download.

    ``BIOENGINE_LOCAL_ARTIFACT_PATH`` (the dev override that
    ``AppBuilder._local_artifact_root`` honoured pre-refactor) short-circuits
    the download and uses a locally mounted artifact root, so unit tests and
    local single-machine runs don't need a live Hypha.
    """
    ...
    with open(lock_path, "w") as lock_fh:
        ...
        try:
            ...
            if not url:
                raise RuntimeError(
                    "BIOENGINE_ARTIFACT_DOWNLOAD_URL not set; replica cannot "
                    "materialise the app source. The worker is expected to "
                    "populate this env_var when constructing the deployment's "
                    "runtime_env."
                )
