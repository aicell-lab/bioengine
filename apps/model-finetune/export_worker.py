...
``micro_sam.bioimageio.export_sam_model``. That call self-tests the package on
CPU (prompt round-trip + image-only AIS) before saving, so a returned package is
already spec-valid and reproducible. We unzip it into ``<export_dir>/package/``,
optionally bake the provenance into ``rdf.yaml``'s ``config``, and write
``export_result.json`` for the entry to stage/upload.
