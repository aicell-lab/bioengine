...
The legacy key is deliberately narrow. Only a *personal* workspace (`ws-user-…`) counts, because Hypha assigns it and it is 1:1 with a human; a shared workspace such as `bioimage-io` is excluded, since aliasing one would promote every member to owner of its records. And anonymous callers get no legacy key at all: every pre-0.11.0 anonymous record was written under the bare string `"anonymous"`, which identifies nobody, so honouring it would hand each anonymous caller the entire old pool and undo the fix above. Those records stay orphaned, which is the correct outcome for data whose owner was never established.
